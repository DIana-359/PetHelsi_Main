...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const petsRes = await fetch(`https://om6auk3tiqy3ih6ad5ad2my63q0xmqcs.lambda-url.eu-north-1.on.aws/api/v1/users/pets`);
        const userPets: Pet[] = petsRes.ok ? await petsRes.json() : [];

        setPets(userPets.map((p, idx) => ({ ...p, checked: true, id: p.id?.toString() || (idx + 1).toString() })));
        setAppointmentData({
          vet: vetData,
          slot: slotData,
  ...
  const formatDateTime = (dateTime: string) => {
    ...
    return {
      ...
      time: date.toLocaleTimeString('uk-UA', { hour: '2-digit', minute: '2-digit' }) + ' (GMT+02:00)'
    };
  };

  if (loading) return <div className="flex justify-center items-center min-h-screen">Завантаження...</div>;
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 p-4 max-w-6xl mx-auto">
      <div className="md:w-1/2 bg-white p-6 rounded-lg shadow">
        ...
        {error && <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-4">⚠️ {error}</div>}
      ...
      <div className="md:w-1/2 bg-white p-6 rounded-lg shadow">
        <div className="text-center mb-6">
          <p className="text-xl font-bold">{timeLeft.minutes} хв : {timeLeft.seconds.toString().padStart(2, '0')} сек</p>
        ...
        {appointmentData && (
          <div className="space-y-4 mb-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>{appointmentData.vet.surname} {appointmentData.vet.name} {appointmentData.vet.patronymic}</div>
              ...
              <div>{selectedIssue !== 'Що турбує тварину?' ? selectedIssue : additionalInfo}</div>
            </div>
          </div>
        )}
